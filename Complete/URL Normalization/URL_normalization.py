...
def checkio(url):
    ...
    url = escape_sequences_upper(url)
    print('Upper escape sequences url:  ', url)

    unreserved_characters = parse_hex_ranges(UNRESERVED_RANGES)
    url = unreserved_characters_decode(url, unreserved_characters)
    print('Decode unreserved characters:', url)

    url = remove_default_ports(url)
    print('Url without default ports:   ', url)

    url = remove_dot_segments(url)
    print('Without dot segments:        ', url)
